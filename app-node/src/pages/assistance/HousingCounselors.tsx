/**
 * Native housing-counselors page (SSR). Ports
 * app/javascript/pages/getAssistance/housing-counselors.tsx.
 *
 * The full counselor list renders server-side; the language/service filter runs
 * client-side (useState/useMemo). window access is confined to onClick handlers,
 * so it's SSR-safe. CounselorFilter is reused directly from the Rails tree.
 */
...
import CounselorFilter from "../../../../app/javascript/pages/getAssistance/counselor-filter"
...
import "../../../../app/javascript/pages/getAssistance/housing-counselors.css"
...
export function HousingCounselors() {
  ...
  const [filterData, setFilterData] = useState<{ language: string; services: string[] }>({
    language: "any",
    services: [],
  })

  const filteredList = useMemo(() => {
    let list = housingCounselorsList.counselors
    if (filterData.language !== "any") {
      list = list.filter((counselor) => counselor.languages.includes(filterData.language))
    }
    if (filterData.services.length > 0) {
      list = list.filter((counselor) =>
        counselor.services.some((service) => filterData.services.includes(service))
      )
    }
    return list
  }, [filterData])
        ...
        <div className="md:mr-8 md:mb-2 md:mt-4">
          ...
          <div className="px-6 pt-6 md:pl-0 md:mt-12">
            <CounselorFilter
              ...
              clearClick={() => setFilterData({ language: "any", services: [] })}
            />
          </div>
