/**
 * Native document-checklist page (SSR). Ports
 * app/javascript/pages/getAssistance/document-checklist.tsx.
 *
 * Differences from the Rails version (SSR-safety):
 *  - No useTranslate (native pages render source copy; machine translation isn't
 *    wired — see migration-status.md).
 *  - The Rails version read window.location to auto-expand the section matching
 *    the URL hash. That's dropped to keep hydration clean; the `id` anchors are
 *    kept so the browser still scrolls to a linked section natively.
 */
import { ContentAccordion, t } from "@uic"
import { getSfGovUrl, renderInlineMarkup } from "../../../../app/javascript/util/languageUtil"
import { PREFERENCES_IDS } from "../../../../app/javascript/modules/constants"
      ...
      expanded: (
        <div className="space-y-2.5">
          ...
          <ol className="list-decimal ml-7">
            <li>
              <p className="pb-2.5">{renderInlineMarkup(t("documentChecklist.assistedHousingDesc"))}</p>
              <ul className="list-disc ml-7">
                <li>
